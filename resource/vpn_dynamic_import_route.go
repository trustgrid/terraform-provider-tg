...
func VPNDynamicImportRoute() *schema.Resource {
	md := majordomo.NewResource(
		majordomo.ResourceArgs[tg.VPNRoute, hcl.VPNRoute]{
			CreateURL: func(r hcl.VPNRoute) string {
				if r.ClusterFQDN != "" {
					return fmt.Sprintf("/v2/cluster/%s/vpn/%s/dynamic/import-route", r.ClusterFQDN, r.NetworkName)
				}
				return fmt.Sprintf("/v2/node/%s/vpn/%s/dynamic/import-route", r.NodeID, r.NetworkName)
			},
			UpdateURL: func(r hcl.VPNRoute) string {
				if r.ClusterFQDN != "" {
					return fmt.Sprintf("/v2/cluster/%s/vpn/%s/dynamic/import-route/%s", r.ClusterFQDN, r.NetworkName, r.UID)
				}
				return fmt.Sprintf("/v2/node/%s/vpn/%s/dynamic/import-route/%s", r.NodeID, r.NetworkName, r.UID)
			},
			...
			OnCreateReply: func(ctx context.Context, args majordomo.CallbackArgs[tg.VPNRoute, hcl.VPNRoute]) (string, error) {
				...
				url := fmt.Sprintf("/v2/node/%s/vpn/%s/dynamic/import-route", args.Resource.NodeID, args.Resource.NetworkName)
				if args.Resource.ClusterFQDN != "" {
					url = fmt.Sprintf("/v2/cluster/%s/vpn/%s/dynamic/import-route", args.Resource.ClusterFQDN, args.Resource.NetworkName)
				}
				...
				return "", fmt.Errorf("dynamic import route not found")
			},
	...
	return &schema.Resource{
		Description: "Manage a VPN dynamic import route on a node or cluster.",
