...
func TestDocsFollowExistingFormat(t *testing.T) {
	...
	containerDoc, err := os.ReadFile(containerDocPath)
	require.NoError(t, err, "Reference documentation file (container.md) should exist")

	referenceDoc := string(containerDoc)

	// Check that both new docs follow the same structure as existing container.md
	...
	for _, docFile := range newDocs {
		t.Run(docFile, func(t *testing.T) {
			...
			// Check for same frontmatter style
			if strings.Contains(referenceDoc, "page_title:") {
				assert.Contains(t, doc, "page_title:", "Should have page_title in frontmatter")
			}

			// Check for same generated comment
			if strings.Contains(referenceDoc, "generated by https://github.com/hashicorp/terraform-plugin-docs") {
			...
			// Check for same schema generated comment
			if strings.Contains(referenceDoc, "<!-- schema generated by tfplugindocs -->") {
...
func assertValidHCL(t *testing.T, hclCode string, exampleNum int) {
	...
	assert.Empty(t, syntaxErrors, "HCL example %d should be syntactically valid: %s", exampleNum, syntaxErrors.Error())
}
...
func TestSchemaDescriptionsArePresent(t *testing.T) {
	...
	for docPath, expectedDescriptions := range docs {
		t.Run(docPath, func(t *testing.T) {
			content, err := os.ReadFile(docPath)
			require.NoError(t, err, "Documentation file should exist: %s", docPath)
